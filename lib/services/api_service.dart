...
class ApiService {
  ...
  Future<Map<String, dynamic>> getCurrentUser(int userId) async {
    final response = await http.get(
      Uri.parse('$baseUrl/user?user_id=$userId'),
    );
  ...
  /// Lấy danh sách tất cả nhiệm vụ của 1 dự án
  Future<List<Map<String, dynamic>>> getAllTasksByProjectId(
      int projectId) async {
    final response = await http.get(
      Uri.parse('$baseUrl/employee/tasks?project_id=$projectId'),
    );
    if (response.statusCode == 200) {
      final List<dynamic> data = json.decode(response.body);
      return data.cast<Map<String, dynamic>>();
  ...
  /// Lấy danh sách tài liệu đính kèm của một dự án
  Future<List<Map<String, dynamic>>> getAttachmentsByProjectId(
      int projectId) async {
    final response = await http.get(
      Uri.parse('$baseUrl/employee/attachments?project_id=$projectId'),
    );
  ...
  /// Lấy các bình luận của một dự án
  Future<List<Map<String, dynamic>>> getCommentsByTaskId(int taskId) async {
    final response = await http.get(
      Uri.parse('$baseUrl/employee/comments?task_id=$taskId'),
    );
  ...
  Future<List<Map<String, dynamic>>> getTasksByEmployeeId(int empoyeeId) async {
  ...
  Future<List<Map<String, dynamic>>> getTaskDetailsByTaskId(int taskId) async {
    final response = await http
        .get(Uri.parse('$baseUrl/employee/task_details?task_id=$taskId'));
    if (response.statusCode == 200) {
      final List<dynamic> data = json.decode(response.body);
      return data.cast<Map<String, dynamic>>();
    } else {
  ...
  /// Lấy danh sách tệp đính kèm theo task
  Future<List<Map<String, dynamic>>> getAttachmentsByTaskDetailId(
      int taskDetailId) async {
    final response = await http.get(
        Uri.parse('$baseUrl/tasks/attachments?task_detail_id=$taskDetailId'));
    if (response.statusCode == 200) {
      final List<dynamic> data = json.decode(response.body);
      return data.cast<Map<String, dynamic>>();
    } else {
  ...
  Future<List<Map<String, dynamic>>> getNotificationsByUserId(
      int userId) async {
    final response = await http.get(
      Uri.parse('$baseUrl/employee/notifications?user_id=$userId'),
    );
  ...
  /// Lấy danh sách hình phạt của một nhiệm vụ
  Future<List<Map<String, dynamic>>> getPenaltiesByTaskId(int taskId) async {
    final response = await http.get(
      Uri.parse('$baseUrl/employee/penalties?task_id=$taskId'),
    );
  ...
  /// Lấy danh sách đánh giá của 1 nhiệm vụ
  Future<List<Map<String, dynamic>>> getEvaluationsByTaskId(int taskId) async {
    final response = await http.get(
      Uri.parse('$baseUrl/employee/evaluations?task_id=$taskId'),
    );
